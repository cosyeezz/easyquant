...
- **架构升级**: 完成了 **全 WebSocket 事件流** 重构。
    - 实现了 `EventService` -> `WebSocketManager` 的直接推送链路，消除了 HTTP 轮询。
    - 确立了 `process.events.{name}` 动态订阅模式，实现了按需、实时的事件监控。
- **代码重构**: 实施了基础设施与业务逻辑的解耦。
    - `websocket_manager.py` 回归纯粹的通信组件。
    - 监控业务逻辑（日志/系统状态）迁移至 `api/v1/system.py`。
- **架构决策**: 
    - **拒绝引入 RocketMQ**: 确认当前 Python 异步内存队列方案在单体架构下具有最低延迟和最优性能，避免过度设计。
    - **拒绝引入 Netty**: 坚持纯 Python (FastAPI/Uvicorn) 技术栈，避免引入 Java 生态导致的维护复杂性。
...
- **交互优化**: 重构前端 `LogViewer` 组件，引入 `localStorage` 持久化用户的“订阅/折叠”偏好，实现了显式的“按需订阅”带宽优化策略。
- **新特性原型**: 引入 `reactflow` 启动 ETL 可视化画布开发，设计了 Schema 传播架构以实现上下文感知的节点配置体验。
- **代码修正**: 修复了 Alembic 迁移脚本 (`60d403367b7a`)，增加了对 PostgreSQL `ENUM` 类型的清理逻辑 (`DROP TYPE IF EXISTS ... CASCADE`)，解决了数据库重建时的类型冲突。
- **当前状态**: 监控系统已达到生产级实时性要求。环境自愈能力已验证。无论数据库是否为空，执行 `python manage.py start` 均可自动完成初始化并提供服务。
