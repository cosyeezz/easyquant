...
*   ✅ **任务 1.4: (核心逻辑重构) 引入管道与处理器模式**
    ...
    *   **多进程并行模式:**
        ...
        3.  **分发:** `Scheduler` 采用**动态批次策略**，将数据源打包分发。批次大小根据 `总任务数 / (Worker数 * 4)` 计算，以在减少调度开销和保持负载均衡之间取得平衡。
        4.  **执行:** **每个独立的工作进程**接收到一批次文件路径后：
            *   **生产者-消费者:** 调用 `loader.stream(sources=batch)`。Loader 在后台并发加载文件（生产者），同时主循环从队列获取 DataFrame（消费者）。
            *   **处理:** 对每个 DataFrame，实例化一个 `Pipeline` 对象，并调用 `pipeline.run(df)` 来执行后续的处理和保存逻辑。
        5.  **总结:** 这种模式下，不仅实现了跨进程并行，还在每个进程内部实现了 IO 和计算的重叠（Overlapping），进一步提升了吞吐量。
