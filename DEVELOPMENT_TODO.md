...
*   ✅ **任务 1.4: (核心逻辑重构) 引入管道与处理器模式**
    ...
    *   **架构设计与并行化策略 (V2.0 - 讨论共识):**
        ...
        *   **组件职责定义:**
            *   **`Loader` (ETL之'E'):** 数据加载器。职责是发现并异步产出需要处理的数据单元（如文件路径列表）。它独立于处理流程。
            *   **`Handler` (处理环节):** 处理器。职责是完成数据处理流程中的一个具体、单一的步骤（如读取文件内容、转换数据、计算指标等）。`Handler` 是可复用的原子操作。
            *   **`Pipeline` (ETL之'T'):** 逻辑管道/编排器。职责是串联多个 `Handler`，定义它们的执行顺序和数据流。`Pipeline` 接收一个初始数据单元，然后将其依次传递给内部的 `Handler` 链进行处理。它工作在**单个进程**的 `asyncio` 事件循环中，自身不感知多进程。
    ...
    *   **多进程并行模式:**
        ...
        2.  **加载:** `Scheduler` 首先调用 `Loader` 获取所有待处理的数据单元（例如1000个文件路径）。
        3.  **分发:** `Scheduler` 创建一个 `multiprocessing.ProcessPoolExecutor` 或类似的进程池。然后，它将获取到的文件路径列表**分发**给池中的多个工作进程。
        4.  **执行:** **每个独立的工作进程**都会接收到一部分文件路径。对于每个路径，工作进程会：
            *   **实例化一个自己的 `Pipeline` 对象。**
            *   调用 `pipeline.run(path)` 来执行完整的、包含了数据读取、处理和最终保存的异步处理流程。
        5.  **总结:** 这种模式下，`Pipeline` 的设计可以完全专注于单进程内的异步逻辑，而 `Scheduler` 专注于跨进程的任务分发。两者职责清晰，互不干扰，实现了“宏观多进程并行，微观异步并发”的高性能模型。
