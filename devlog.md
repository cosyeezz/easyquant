...
### 3. 系统维护
- **README**: 更新了项目文档，反映了最新的架构变化（ETL 模块、API 列表）。
- **GEMINI.md**: 确认并遵守了相关开发约定。

## 下一步计划
1.  **实现 `DatabaseSaveHandler`**: 完成数据入库逻辑。
2.  **实现 Runner**: 编写 `server/etl/runner.py`，串联 Loader、Pipeline 和 Event Service，实现真正的任务执行。
3.  **实现 Run API**: `POST /api/v1/etl/etl-configs/:id/run`。